...
public:
    ...
    virtual std::unique_ptr<layer_fwd> build_fwd(
        mkldnn::memory::dims& src_tz,
        mkldnn::memory& src_memory,
        mkldnn::memory::desc& src_md,
        mkldnn::engine& cpu_engine) = 0;
    virtual std::unique_ptr<layer_bwd> build_bwd(
...
public:
    ...
    std::unique_ptr<layer_fwd> build_fwd(
        mkldnn::memory::dims& src_tz,
        mkldnn::memory& src_memory,
        mkldnn::memory::desc& src_md,
        mkldnn::engine& cpu_engine);
    std::unique_ptr<layer_bwd> build_bwd(
        mkldnn::memory& diff_dst_memory,
        mkldnn::memory& src_memory,
        mkldnn::memory::desc& src_md,
        mkldnn::memory::dims& src_tz,
