...
convolution_layer_fwd::convolution_layer_fwd(mkldnn::memory::dims& src_tz, mkldnn::memory& src_memory, mkldnn::memory::desc& src_md, mkldnn::engine& cpu_engine, int kernel, int dims, int padding, int stride)
        : conv_weights_tz({dims, src_tz[1], kernel, kernel})
        , conv_bias_tz({dims})
        , dst_tz({src_tz[0], dims, (src_tz[2] + padding*2 + stride - kernel)/stride, (src_tz[2] + padding*2 + stride - kernel)/stride})
        , conv_strides({stride, stride})
        , conv_padding({padding, padding})
        , conv_weights(std::accumulate(conv_weights_tz.begin(), conv_weights_tz.end(), 1, std::multiplies<uint32_t>()), 1.)
...
relu_layer_fwd::relu_layer_fwd(mkldnn::memory::dims& src_tz, mkldnn::memory& src_memory, mkldnn::memory::desc& src_md, mkldnn::engine& cpu_engine)
        : dst_tz(src_tz)
...
softmax_layer_fwd::softmax_layer_fwd(mkldnn::memory::dims& src_tz, mkldnn::memory& src_memory, mkldnn::memory::desc& src_md, mkldnn::engine& cpu_engine)
        : dst_tz(src_tz)
...
relu_layer_bwd::relu_layer_bwd(mkldnn::memory& diff_dst_memory, mkldnn::memory& src_memory, mkldnn::memory::desc& src_md, mkldnn::memory::dims& src_tz, layer_fwd* fwd, mkldnn::engine& cpu_engine)
...
softmax_layer_bwd::softmax_layer_bwd(mkldnn::memory& diff_dst_memory, mkldnn::memory& src_memory, mkldnn::memory::desc& src_md, mkldnn::memory::dims& src_tz, layer_fwd* fwd, mkldnn::engine& cpu_engine)
        : diff_dst_md(diff_dst_memory.get_primitive_desc().desc())
...
convolution_layer_bwd::convolution_layer_bwd(mkldnn::memory& diff_dst_memory, mkldnn::memory& src_memory, mkldnn::memory::desc& src_md, mkldnn::memory::dims& src_tz, layer_fwd* fwd, mkldnn::engine& cpu_engine)
        : conv_user_diff_weights_buffer(std::accumulate(fwd->conv_weights_tz_()->begin(), fwd->conv_weights_tz_()->end(), 1, std::multiplies<uint32_t>()), 0.)
        , conv_diff_bias_buffer(std::accumulate(fwd->conv_bias_tz_()->begin(), fwd->conv_bias_tz_()->end(), 1, std::multiplies<uint32_t>()), 0.)
        ...
        , conv_diff_bias_md(mkldnn::memory::desc({*fwd->conv_bias_tz_()}, mkldnn::memory::data_type::f32, mkldnn::memory::format::any))
        , conv_diff_weights_md(mkldnn::memory::desc({*fwd->conv_weights_tz_()}, mkldnn::memory::data_type::f32, mkldnn::memory::format::any))
        , conv_diff_dst_md(mkldnn::memory::desc({fwd->dst_tz_()}, mkldnn::memory::data_type::f32, mkldnn::memory::format::any))
        , conv_bwd_weights_desc(mkldnn::convolution_backward_weights::desc(mkldnn::convolution_direct, conv_bwd_src_md, conv_diff_weights_md,
                                                                           conv_diff_bias_md, conv_diff_dst_md, *fwd->conv_strides_(), *fwd->conv_padding_(), *fwd->conv_padding_(), mkldnn::padding_kind::zero))
        ...
        , conv_bwd_desc(mkldnn::convolution_direct, conv_bwd_src_md, conv_diff_weights_md, conv_diff_dst_md, *fwd->conv_strides_(),
                        *fwd->conv_padding_(), *fwd->conv_padding_(), mkldnn::padding_kind::zero)
